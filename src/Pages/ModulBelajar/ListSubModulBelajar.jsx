...
import { MdLibraryBooks, MdSubdirectoryArrowRight } from "react-icons/md";
import { BsCollection } from "react-icons/bs";

const ListSubModulBelajar = () => {
  const [subModulList, setSubModulList] = useState([]);
  const [modulInfo, setModulInfo] = useState(null);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [selectedSubModul, setSelectedSubModul] = useState(null);
  ...
  const fetchSubModulList = async () => {
    try {
      ...
      setSubModulList(response.data);
      console.log(response.data);

      // Set modul info from first sub modul (since all sub moduls belong to same modul)
      if (response.data.length > 0 && response.data[0].modul) {
        setModulInfo(response.data[0].modul);
      }
    } catch (error) {
      console.error("Error fetching sub modul list:", error);
      if (error.response?.status === 404) {
        setError("Belum ada sub modul untuk modul ini");
      } else if (error.response?.status === 400) {
        setError("ModulId tidak valid");
        setTimeout(() => navigate("/modul-belajar"), 2000);
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`${apiUrl}/sub-modul/${selectedSubModul.id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      setSubModulList(
        subModulList.filter((subModul) => subModul.id !== selectedSubModul.id)
      );
      setShowDeleteModal(false);
      setSelectedSubModul(null);
    } catch (error) {
      console.error("Error deleting sub modul:", error);
      setError("Gagal menghapus sub modul");
    } finally {
      setIsDeleting(false);
    }
  };

  const filteredSubModuls = subModulList.filter(
    (subModul) =>
      subModul.subJudul?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      subModul.subDeskripsi?.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  >
                    <span style={{ color: currentColor }}>Sub Modul</span>{" "}
                    Belajar
                  </h1>
                  <p
                  ...
                  >
                    {modulInfo
                      ? `Modul: ${modulInfo.judul}`
                      : "Koleksi Sub Modul Pembelajaran"}
                  </p>
                </div>
              </div>
              {user?.role !== "siswa" && (
        ...
        >
          {loading ? (
            ...
            >
              ...
              >
                {searchTerm
                  ...
                  : error || "Belum ada sub modul"}
              </h3>
              <p
                className={`${isDark ? "text-gray-400" : "text-gray-600"} mb-6`}
              >
                {searchTerm
                  ? `Tidak ada sub modul yang cocok dengan "${searchTerm}"`
                  : error ||
                    "Mulai dengan menambahkan sub modul pertama untuk modul ini"}
              </p>
              {!searchTerm && !error && (
                <motion.button
                  whileHover={{ scale: 1.05 }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              <AnimatePresence>
                {filteredSubModuls.map((subModul, index) => (
                  <motion.div
                    key={subModul.id}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    ...
                    <div className="relative h-48 overflow-hidden">
                      ...
                      <div className="absolute top-3 left-3">
                        <span
                          ...
                          style={{ backgroundColor: currentColor }}
                        >
                          <MdSubdirectoryArrowRight className="inline mr-1" />
                          Sub Modul
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="p-4">
                      <div className="mb-4">
                        ...
                        >
                          {truncateText(subModul.subJudul, 40)}
                        </h3>
                        <p
                        ...
                        >
                          {truncateText(subModul.subDeskripsi, 80)}
                        </p>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <motion.button
                          ...
                          whileTap={{ scale: 0.95 }}
                          onClick={() =>
                            navigate(`/sub-modul-belajar/view/${subModul.id}`)
                          }
                          className={`${
                        ...
                        >
                          ...
                          {user?.role === "siswa" ? "Pelajari" : "Detail"}
                        </motion.button>
                        ...
                        {user?.role !== "siswa" && (
                          <>
                            <motion.button
                              ...
                              onClick={() =>
                                navigate(
                                  `/sub-modul-belajar/edit/${subModul.id}`
                                )
                              }
          ...
          >
            ...
            >
              ...
              <div className="p-6">
                ...
                >
                  Apakah Anda yakin ingin menghapus sub modul "
                  {selectedSubModul?.subJudul}"? Tindakan ini tidak dapat
                  dibatalkan.
                </p>
